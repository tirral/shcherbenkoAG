...
if (!function_exists('shcherbenko_setup')) :
    ...
    function shcherbenko_setup()
    {
        ...
        register_nav_menus(array(
            ...
            'menu-3' => esc_html__('Myxi', 'shcherbenko'),
        ));
...
function shcherbenko_admin()
{
    wp_enqueue_style("style-admin", get_bloginfo('stylesheet_directory') . "/css/style-admin.css");
    wp_enqueue_script('script-admin', get_template_directory_uri() . '/js/admin.js');
}

add_action('admin_head', 'shcherbenko_admin');
...
function artists_rewrites()
{
    ...
    // Страница со списком постов терма определенного типа поста с номером страницы для пагинации
    add_rewrite_rule('^(project)/([^/]*)/page/([0-9]+)/?', 'index.php?page_type=post_project_archive_by_term&term_post_type=$matches[1]&taxonomy=artists&term=$matches[2]&paged=$matches[3]', 'top');
    ...
    add_rewrite_rule('^artists/page/([0-9]+)/?', 'index.php?page_type=term_archive&taxonomy=artists&paged=$matches[1]', 'top');
